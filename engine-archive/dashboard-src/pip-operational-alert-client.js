...
import {
  validatePipOperationalAlertAcceptanceReceipt,
  validatePipOperationalAlertAcknowledgementReceipt,
  validatePipOperationalAlertReport,
} from "./pip-operational-alert-contract.js";
...
function makeClientError(message, details = {}) {
  const error = new Error(
    String(message ?? "Operational alert request failed.")
  );
  error.status = Number(details.status ?? 0);
  error.code = String(details.code ?? "OPERATIONAL_ALERT_REQUEST_FAILED");
...
async function parseJsonResponse(response) {
  ...
  try {
    ...
    throw makeClientError("Malformed JSON response.", {
      ...
      code: "MALFORMED_OPERATIONAL_ALERT_RESPONSE_JSON",
      request_id:
...
function parseSuccessEnvelope(payload, response, endpoint) {
  if (!isPlainObject(payload)) {
    throw makeClientError("Malformed success envelope.", {
      status: response.status,
      code: "MALFORMED_OPERATIONAL_ALERT_SUCCESS_ENVELOPE",
      request_id:
        response.headers.get("x-pip-request-id") ?? null,
      details: { endpoint },
    });
  ...
  if (!valid) {
    throw makeClientError("Malformed success envelope.", {
      status: response.status,
      code: "INVALID_OPERATIONAL_ALERT_SUCCESS_ENVELOPE",
      request_id:
        payload.request_id ??
...
function throwErrorEnvelope(payload, response, endpoint) {
  if (!isPlainObject(payload)) {
    throw makeClientError("Malformed error envelope.", {
      status: response.status,
      code: "MALFORMED_OPERATIONAL_ALERT_ERROR_ENVELOPE",
      request_id:
        response.headers.get("x-pip-request-id") ?? null,
      details: { endpoint },
    });
  ...
  if (!valid) {
    throw makeClientError("Malformed error envelope.", {
      status: response.status,
      code: "INVALID_OPERATIONAL_ALERT_ERROR_ENVELOPE",
      request_id:
        payload.request_id ??
        response.headers.get("x-pip-request-id") ??
        null,
  ...
  throw makeClientError(
    String(payload.error.message ?? "Operational alert request failed."),
    {
      status: response.status,
  ...
  return fetchImpl(request.url, {
    ...
    .catch((error) => {
      if (error?.name === "AbortError") {
        throw makeClientError("Operational alert request timed out.", {
          status: 0,
          code: "OPERATIONAL_ALERT_REQUEST_TIMEOUT",
          details: { endpoint: request.endpoint, timeout_ms: timeoutMs },
        });
      }

      throw makeClientError("Operational alert connection failed.", {
        status: 0,
        code: "OPERATIONAL_ALERT_CONNECTION_FAILED",
...
export function createPipOperationalAlertClient({
  baseUrl,
  fetchImpl,
  timeoutMs,
  getCsrfToken,
  ...
  if (typeof resolvedFetch !== "function") {
    throw new Error(
      "PIP operational alert client requires a fetch implementation."
    );
  }
  ...
  return {
    async getReport() {
      const response = await request({
        method: "GET",
        endpoint: "/operational-alerts",
      });

      const validation = validatePipOperationalAlertReport(response.data);
      if (validation.valid !== true) {
        throw makeClientError(
          validation.errors?.[0] ??
            "Invalid operational alert report payload.",
          {
            status: response.status,
            code: "INVALID_OPERATIONAL_ALERT_REPORT_PAYLOAD",
    ...
    async acknowledgeAlert({
      ...
      const response = await request({
        method: "POST",
        endpoint: "/operational-alerts/acknowledge",
        requiresCsrf: true,
        body: {
          alertFingerprint,
          constituencyKey,
      ...
      if (validation.valid !== true) {
        throw makeClientError(
          validation.errors?.[0] ??
            "Invalid operational alert acknowledgement receipt.",
          {
            status: response.status,
            code: "INVALID_OPERATIONAL_ALERT_ACKNOWLEDGEMENT_RECEIPT",
            request_id: response.request_id,
            details: { checks: validation.checks },
          }
        );
    ...
    async runAcceptance({
      ...
      const response = await request({
        method: "POST",
        endpoint: "/operational-alerts/acceptance",
        requiresCsrf: true,
        body: {
      ...
      const validation =
        validatePipOperationalAlertAcceptanceReceipt(response.data);
      if (validation.valid !== true) {
        throw makeClientError(
          validation.errors?.[0] ??
            "Invalid operational alert acceptance receipt.",
          {
            status: response.status,
            code: "INVALID_OPERATIONAL_ALERT_ACCEPTANCE_RECEIPT",
            request_id: response.request_id,
            details: { checks: validation.checks },
          }
        );
