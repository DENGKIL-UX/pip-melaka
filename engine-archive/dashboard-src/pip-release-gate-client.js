...
import {
  validatePipReleaseGateAcceptanceReceipt,
  validatePipReleaseGateReport,
} from "./pip-release-gate-contract.js";
...
function makeClientError(message, details = {}) {
  const error = new Error(String(message ?? "Release-gate request failed."));
  error.status = Number(details.status ?? 0);
  error.code = String(details.code ?? "RELEASE_GATE_REQUEST_FAILED");
...
async function parseJsonResponse(response) {
  ...
  try {
    ...
    throw makeClientError("Malformed JSON response.", {
      ...
      code: "MALFORMED_RELEASE_GATE_RESPONSE_JSON",
...
function parseSuccessEnvelope(payload, response, endpoint) {
  if (!isPlainObject(payload)) {
    throw makeClientError("Malformed success envelope.", {
      status: response.status,
      code: "MALFORMED_RELEASE_GATE_SUCCESS_ENVELOPE",
      request_id: response.headers.get("x-pip-request-id") ?? null,
      details: { endpoint },
    });
  }
  ...
  if (!valid) {
    throw makeClientError("Malformed success envelope.", {
      status: response.status,
      code: "INVALID_RELEASE_GATE_SUCCESS_ENVELOPE",
      request_id:
...
function throwErrorEnvelope(payload, response, endpoint) {
  if (!isPlainObject(payload)) {
    throw makeClientError("Malformed error envelope.", {
      status: response.status,
      code: "MALFORMED_RELEASE_GATE_ERROR_ENVELOPE",
      request_id: response.headers.get("x-pip-request-id") ?? null,
      details: { endpoint },
    });
  }
  ...
  if (!valid) {
    throw makeClientError("Malformed error envelope.", {
      status: response.status,
      code: "INVALID_RELEASE_GATE_ERROR_ENVELOPE",
      request_id:
        payload.request_id ?? response.headers.get("x-pip-request-id") ?? null,
      details: { endpoint },
    });
  ...
  throw makeClientError(
    String(payload.error.message ?? "Release-gate request failed."),
    {
      status: response.status,
  ...
  return fetchImpl(request.url, {
    ...
    .catch((error) => {
      if (error?.name === "AbortError") {
        throw makeClientError("Release-gate request timed out.", {
          status: 0,
          code: "RELEASE_GATE_REQUEST_TIMEOUT",
          details: {
      ...
      throw makeClientError("Release-gate connection failed.", {
        status: 0,
        code: "RELEASE_GATE_CONNECTION_FAILED",
        details: {
          endpoint: request.endpoint,
        },
      });
...
export function createPipReleaseGateClient({
  baseUrl,
  fetchImpl,
  timeoutMs,
  getCsrfToken,
  ...
  if (typeof resolvedFetch !== "function") {
    throw new Error("PIP release-gate client requires a fetch implementation.");
  }
  ...
  return {
    async getReport({ targetEnvironment } = {}) {
      ...
      const response = await request({
        method: "GET",
        endpoint: `/release-gate${suffix}`,
      });

      const validation = validatePipReleaseGateReport(response?.data);
      if (validation.valid !== true) {
        throw makeClientError(
          validation.errors?.[0] ?? "Release-gate report validation failed.",
          {
            status: response.status,
            code: "RELEASE_GATE_REPORT_VALIDATION_FAILED",
    ...
    async runAcceptance({ targetEnvironment, constituencyKey, parliamentCode } = {}) {
      const response = await request({
        method: "POST",
        endpoint: "/release-gate/acceptance",
        requiresCsrf: true,
        body: {
          targetEnvironment,
          constituencyKey,
      ...
      const validation = validatePipReleaseGateAcceptanceReceipt(response?.data);
      if (validation.valid !== true) {
        throw makeClientError(
          validation.errors?.[0] ??
            "Release-gate acceptance validation failed.",
          {
            status: response.status,
            code: "RELEASE_GATE_ACCEPTANCE_VALIDATION_FAILED",
            request_id: response.request_id,
          }
        );
      }
